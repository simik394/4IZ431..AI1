...
```{julia}
#| label: render-pragmatic-tree
#| eval: true
#| echo: false
#| output: asis

# ── Generování stromu pro Pragmatic variantu ─────────────────────────
# Chceme vizualizovat pouze "manuální" simulaci = čistou vítěznou cestu
# Použijeme nastavení 10,14 vs 1 a zapneme logování stromu s ořezáváním

# Suppress debug output from includes and calculation
redirect_stdio(stdout=devnull) do
    # Setup
    include(joinpath(cur_doc_dir, "testvaluefunc.jl"))
    
    # Inline setup to avoid World Age issues
    board = zeros(Int, 8, 8)
    # We use invokelatest for notation_to_position just in case needed, but usually inline is fine if included
    # actually, purely inline might still have issues if include happens in same block.
    # But let's try direct calls.
    ...
    # Použijeme minimax_with_tree s PRUNE_HUMAN (3) - Beam Search K=2
    # To zajistí malý, čitelný strom (~13 uzlů) pro vizualizaci principu
    # Note: We must pass the Enum type, not just Int
    Base.invokelatest(minimax_with_tree, board, 6, -Inf, Inf, true, 0, "Start"; pruning=PruningStrategy(3))

    # Export do DOT
    tree_out_dir = joinpath(cur_doc_dir, "..", "out", "rendered_trees", "pragmatic")
    mkpath(tree_out_dir)
    dot_file = joinpath(tree_out_dir, "manual_sim.dot")

    # Využijeme funkci z testvaluefunc.jl
    Base.invokelatest(export_tree_to_dot, dot_file)

    # Vykreslení
    png_file = joinpath(tree_out_dir, "manual_sim.png")
    run(`dot -Tpng -Gsize=10,6! $dot_file -o $png_file`)
end
...
println("### Vizualizace rozhodování (Human/Manual Logic)\n")
println("Tento strom ukazuje **pouze** větve, které algoritmus reálně musel vyhodnotit (po aplikaci Alpha-Beta a 'Human' ořezání).")
println("Z původních 2000+ uzlů (Brute Force) zbylo jen **$(length(tree_nodes))**, což odpovídá lidské 'manuální' kontrole - ignorujeme vše, co vede k okamžité ztrátě.")
println("\n![Human Tree (Manual Logic View)]($(relpath(png_file, cur_doc_dir))){width=90%}\n")
```
