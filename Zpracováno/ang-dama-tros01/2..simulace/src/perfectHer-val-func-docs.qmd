...
board = setup_benchmark_board() # Použijeme funkci definovanou v předchozím bloku (nutno zajistit pořadí)
# Nebo ji znova definujeme pro jistotu, pokud bloky nesdílí scope (Quarto usually shares but depends on config)
function setup_viz_board()
    board = zeros(Int, 8, 8)
...
# Použijeme minimax_with_tree s PRUNE_LOSS_OF_PIECE (Pragmatic)
minimax_with_tree(board, 6, -Inf, Inf, true, 0, "Start"; pruning=PRUNE_LOSS_OF_PIECE)
...
# Zobrazení
println("### Vizualizace rozhodování (Pragmatic/Manual Logic)\n")
println("Tento strom ukazuje **pouze** větve, které algoritmus reálně musel vyhodnotit (po aplikaci Alpha-Beta a 'Loss of Piece' ořezání).")
...
println("\n![Pragmatic Tree (Minimalist View)]($(relpath(png_file, cur_doc_dir))){width=100%}\n")
```
