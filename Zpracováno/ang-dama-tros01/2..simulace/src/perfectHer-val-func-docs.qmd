    ...
    function tree_to_text(nodes, max_display_depth::Int, total_depth::Int)
        ...
        function print_subtree(node_id, indent)
            ...
            println(buf, prefix, "├─ ", node.move_str, " (", player,
                ...
                " alpha=", round(node.alpha, digits=1),
                " beta=", round(node.beta, digits=1), prune)
...
# 1. Měření
t0 = time();
_, _, n_bf = minimax_smart_bench(board, depth, true);
t_bf = time() - t0;
t0 = time();
...
1.  **Metrika vzdálenosti**: Původní metrika nedokázala správně ohodnotit rozdíl mezi vyhranou a remízovou variantou, protože ignorovala diagonální dynamiku dámy.
