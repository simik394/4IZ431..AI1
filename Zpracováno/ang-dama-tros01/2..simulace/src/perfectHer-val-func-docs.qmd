...
```{.julia include="/home/sim/Obsi/Prods/04-škola/Předměty/mgr3/4IZ431..AI1/Zpracováno/ang-dama-tros01/2..simulace/src/heuristics.jl" region="perfect_material" dedent="true"}
```
...
1. Penalta: Červený král v bezpečné zóně (dvojitý roh) — heuristika *chce*, aby se soupeř pohnul.
2. Bonus: Červený je mimo bezpečí — indikátor úspěšného vytlačení.
...
1. Bonus za koordinaci králů ve vzdálenosti 2–6 polí (sweet spot).
2. Sevření (Squeeze) — bonus roste s klesající průměrnou vzdáleností k soupeři.
...
1. Tvrdá penalta: oba králové příliš daleko (`avg_dist > 5`) — promarněná příležitost.
2. Střední penalta: vzdálenost nad 4 — agent ztrácí tlak.
3. Penalta za nejbližšího krále stále daleko — špatná pozice.
...
1. Net bonus: Operátor je na správné diagonále (row ≥ 3, col ≥ 4) — síť je roztažená.
2. Crowding penalta: Operátor příliš blízko rohu — králové si překáží.
...
```{.julia include="/home/sim/Obsi/Prods/04-škola/Předměty/mgr3/4IZ431..AI1/Zpracováno/ang-dama-tros01/2..simulace/src/heuristics.jl" region="perfect_mobility" dedent="true"}
```
...
```{.julia include="/home/sim/Obsi/Prods/04-škola/Předměty/mgr3/4IZ431..AI1/Zpracováno/ang-dama-tros01/2..simulace/src/heuristics.jl" region="perfect_cornering" dedent="true"}
```

### PRINCIP 8: Kontextove zavisle rizeni rohu

```{.julia include="/home/sim/Obsi/Prods/04-škola/Předměty/mgr3/4IZ431..AI1/Zpracováno/ang-dama-tros01/2..simulace/src/heuristics.jl" region="perfect_corner_control" dedent="true"}
```
...
```{.julia include="/home/sim/Obsi/Prods/04-škola/Předměty/mgr3/4IZ431..AI1/Zpracováno/ang-dama-tros01/2..simulace/src/testvaluefunc.jl" region="move_ordering" dedent="true"}
```
...
# Absolutni cesta k simulation_data (robustni vuci CWD zmenam pri book renderu)
sim_base_path = "/home/sim/Obsi/Prods/04-škola/Předměty/mgr3/4IZ431..AI1/Zpracováno/simulation_data"

if !isdir(sim_base_path)
    println("⚠ CHYBA: Adresář s daty simulace nenalezen: $sim_base_path")
    # Vytvorime dummy run aby zbytek scriptu nespadl
    mkpath(joinpath(sim_base_path, "run_dummy"))
    open(joinpath(sim_base_path, "run_dummy", "průběh_simulace.txt"), "w") do f
        write(f, "Dummy simulation run\n")
    end
end

run_dirs = filter(d -> startswith(d, "run_"), readdir(sim_base_path))
if isempty(run_dirs)
    println("⚠ Žádné běhy simulace nenalezeny v $sim_base_path")
    mkpath(joinpath(sim_base_path, "run_dummy"))
    open(joinpath(sim_base_path, "run_dummy", "průběh_simulace.txt"), "w") do f
        write(f, "Dummy simulation run\n")
    end
...
println("**Zdrojovy run:** `$latest_run` (z `$sim_base_path`)\n")

## ── Parsovani pruběh_simulace.txt ────────────────────────────────────
raw = read(joinpath(active_run_path, "průběh_simulace.txt"), String)
lines = split(raw, "\n")

## Extrakce zadani
for l in lines
    # Match "Bílý: 3 královen na pozicích 10, 14, 15"
    # Note: Regex must match unicode chars properly
...
redirect_stdio(stdout=devnull, stderr=devnull) do
    ...
    function generate_text_tree(strat_int, depth, display_depth)
        ...
        out_dir = "/home/sim/Obsi/Prods/04-škola/Předměty/mgr3/4IZ431..AI1/Zpracováno/ang-dama-tros01/2..simulace/out/rendered_trees"
        return (total, text)
    end
...
## Load log file
log_path = "/home/sim/Obsi/Prods/04-škola/Předměty/mgr3/4IZ431..AI1/Zpracováno/ang-dama-tros01/2..simulace/out/verification/verification_log.txt"
...
Výše uvedené testy potvrzují, že `perfect_endgame_heuristic` spolehlivě nalézá vítěznou strategii v koncovce 2 králové proti 1 králi. Klíčové závěry:

- **Alpha-Beta ořezávání** dává identické výsledky jako hrubá síla (Brute-force), ale s řádově menším počtem prozkoumaných uzlů.
- **Pseudo-terminální stavy** (ztráta figury, ústup) efektivně zmenšují prohledávací prostor bez ztráty kvality řešení.
- **Beam search (Human varianta)** redukuje strom na pouhých ~13 uzlů a stále nalézá optimální tah — což odpovídá intuitivnímu lidskému rozhodování.

Detailní rozbor regrese (problém nerozhodnosti mezi tahy `14-9` a `10-7`) a její řešení pomocí Čebyševovy metriky je popsán v předchozí kapitole (Případová studie: Rozhodování v Hloubce 6).
